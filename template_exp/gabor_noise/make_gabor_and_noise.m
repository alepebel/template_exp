function [patchimg,noiseimg, cfg] = make_gabor_noise(cfg)
...
%    * envscale - scale of the envelepo (useful to sharpen the border
...
if ~isfield(cfg,'envscale')
    % set random phase to Gabor pattern
     cfg.envscale = 1; % envelop scale
end
...
envscale = cfg.envscale; % scale of the envelope
...
%circular_mask = sqrt((x-0).^2+(y-0).^2)<= diameter/2;
%imagesc(circular_mask);

envelop = (normpdf(r,0,patchenv)/normpdf(0,0,patchenv))*envscale;
% Truncating envelop: make 1 values higher than 1 in envelop.
envelop(envelop > 1) = 1;

% make noisy Gabor patch

patchimg = (gaborimg+noiseimg).* envelop;   %applying envelop
%imagesc(patchimg );
% not lets create just the noisematrix
noiseimg = (noiseimg.* envelop)+patchlum;
